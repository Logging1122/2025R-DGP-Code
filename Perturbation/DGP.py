...
def perturb_domain(df):
    """
        Calculate trajectory dataset perturbation points set
        Returns:
        - points_domain
    """
    df_domain = df.drop_duplicates(subset=['Latitude', 'Longitude'])
    points_domain = df_domain[['Latitude', 'Longitude']].values.tolist()
...
def traj_point_perturb_region(pre_point,current_point,epsilon,trajectory_data,angle_matrix, depth=0):
    """
    ...
    noise_angle = square_wave_mechanism(angle,epsilon)
    # print(current_point)
    ...
    candidates = list(filter(lambda item: item in trajectory_data, angle_candidates))
    ...
    #     return traj_point_perturb_region(pre_point,current_point,epsilon,trajectory_data, angle_matrix,result_label ,l, depth+1)
    # else:
    #     return candidates

def Perturb_traj_points(trajectory,epsilon, trajectory_data, angle_matrix):
    """
    ...
    point_eps = 0.5*epsilon / len(trajectory)
    ...
    for i in range(1, len(trajectory)):
        point = trajectory[i]
        candidates = traj_point_perturb_region(perturb_traj[i-1], point, angle_eps, trajectory_data, angle_matrix)
...
def get_NE_traj_perurb(UID,points_domain, angle_matrix ,epsilon):
    ID_distances = 0
    for ID in UID:
        ...
        perturb_traj = Perturb_traj_points(trajectory, epsilon, points_domain, angle_matrix)
...
def Normalized_Error(run_round, traj_points, df, epsilon):
    ...
    angle_matrix = points_angle_dis_martix(points_domain)
    UID = df['User ID'].unique()
    round_error = 0
    for r in range(0, run_round):
        # ID_distances = get_NE_traj_perurb(UID,points_domain, angle_matrix, epsilon,threshold_value)
        ID_distances = get_NE_traj_perurb(UID, points_domain, angle_matrix, epsilon)
...
def get_PRQ_traj_perturb(UID,points_domain, angle_matrix , epsilon, dist):
    norm_error = 0
    for ID in UID:
        haversine_v = np.vectorize(haversine)
        ID_distances = 0
...
def Preservation_range_query_error(run_round, traj_points, df, epsilon):
    ...
    for dist in dists:
        round_error = 0
        for r in range(0, run_round):
            norm_error = get_PRQ_traj_perturb(UID, points_domain, angle_matrix, epsilon, dist)
...
def research_hotspot(df,POIs):
    ...
    # 选择population最大前POI个，相同的值选择第一个
    real_top = result.nlargest(POIs, 'user_count', keep="first")

    return real_top

def get_ACD_traj_perturb(UID,points_domain, angle_matrix ,epsilon, real_top):
...
def Average_count_distance( traj_points, df, epsilon,run_round,POIs):
    ...
    round_error = 0
    for r in range(0, run_round):
        error_sum = get_ACD_traj_perturb(UID,points_domain, angle_matrix ,epsilon, real_top)
        print(r, 'th round error is ', error_sum)
        round_error += error_sum
    ACD = round_error / run_round
    return ACD

def get_KT_traj_perturb(UID, points_domain, angle_matrix, epsilon,real_top):
    real_top_points = real_top[['Latitude', 'Longitude']].round(6).apply(tuple, axis=1).tolist()
    real_point_set = set(real_top_points)
...
def Kendall_Tau( traj_points, df, epsilon,run_round,POIs):
    ...
    round_error = 0
    for r in range(0, run_round):
        error = get_KT_traj_perturb(UID, points_domain, angle_matrix, epsilon,real_top)
...
if __name__ == "__main__":
       Path = 'E:\\Code\\Trace-2\\dataset\\NYC\\'
       df = pd.read_csv(Path + 'NYC_new.csv', encoding='unicode_escape')
       ...
       #     NE = Normalized_Error(5, traj_points, df,4, threshold_value)
